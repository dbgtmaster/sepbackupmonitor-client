...
bool TcpCommandFactories::registerFactory(TcpAbstractCommandFactory* factory) {
    ...
    if (!insertActionStatus.second) {
        logDebug("TcpCommandFactories::addFactory: Factory already exists!!");
    }
    else {
...
TcpAbstractCommand* TcpCommandFactories::getCommand(const QByteArray& group, const QByteArray& action) const {
    ...
    return itAction->second->getCommand();
}
